...
func (c *P2PClient) performHolePunching() error {
	...
	targets := c.buildPunchTargets()
	if len(targets) == 0 {
		return fmt.Errorf("peer addresses are not available yet")
	}

	fmt.Println("Punch candidates:")
	for idx, cand := range targets {
		fmt.Printf("  %d) %s:%d\n", idx+1, cand.IP, cand.Port)
	}

	stopPunch := make(chan struct{})
	defer close(stopPunch)
	...
	go func() {
		for i := 0; i < 40; i++ {
			...
			for _, cand := range targets {
				if _, err := c.localConn.WriteToUDP(message, cand); err != nil {
					log.Printf("Failed to send hole punch %d to %s:%d: %v", i, cand.IP, cand.Port, err)
					continue
				}
				if i < 5 || i%5 == 0 {
...
func (c *P2PClient) isTargetAddr(addr *net.UDPAddr) bool {
	if addr == nil {
		return false
	}
	...
	if c.peerLocalAddr != nil && addr.IP.Equal(c.peerLocalAddr.IP) {
		return true
	}
	return c.peerAddr == nil
}

func (c *P2PClient) updatePeerRoute(addr *net.UDPAddr) {
	if addr == nil {
		return
	}
	...
	go func() {
		for {
			...
			if n > 0 && buffer[0] == P2P_MESSAGE {
				if c.isTargetAddr(addr) || c.peerAddr == nil {
					if c.peerAddr == nil || !udpAddrsEqual(c.peerAddr, addr) {
						c.peerAddr = addr
					}
...
func (c *P2PClient) buildPunchTargets() []*net.UDPAddr {
	var targets []*net.UDPAddr

	if c.peerLocalAddr != nil {
		if c.localIP != nil && same24(c.localIP, c.peerLocalAddr.IP) {
			targets = append(targets, cloneUDPAddr(c.peerLocalAddr))
		}
	}

	if c.peerAddr != nil {
		targets = append(targets, cloneUDPAddr(c.peerAddr))
	}

	if c.peerLocalAddr != nil {
		alreadyPresent := false
		for _, t := range targets {
			if udpAddrsEqual(t, c.peerLocalAddr) {
				alreadyPresent = true
				break
			}
		}
...
func cloneUDPAddr(addr *net.UDPAddr) *net.UDPAddr {
	...
	ipCopy := append(net.IP(nil), addr.IP...)
	return &net.UDPAddr{IP: ipCopy, Port: addr.Port, Zone: addr.Zone}
}
